     ...
     +     bx,by,bth,alpha0,rate00,desync,D1,torque,xp,yp,b,seed)
      ...
      if(mod(k,prodskip).eq.0) then
         write(1,*) N
         do i=1,N                        
            write(1,'(6E20.6,I10)') x(i),y(i),th(i),d(i),
     ...
     +              d(i)*alpha(i),depth(i),c(i)
         ...
         if(mod(k,prodskip).eq.0) then
            write(*,'(6I10,E20.6)')k,Nsum,N,Nforce,ntUL,ntDL,V/dble(N)
         endif
     ...
     +        bounddepth,vx,vy,vth,ax,ay,ath,bx,by,bth)
         if(N.gt.Nprev) then 
c
c   add to remove_cells - forcelist, traillist, countn, nl
c
...
c            write(*,*)
     ...
     +           bx,by,bth,forcelist,traillist,xp,yp,nl,countn,
     +           bound)
         end if
...
c      do i=1,N
c         write(*,*) i, keeppart(i), renumber(i)
c      enddo
c
c      write(*,*) 
c
c      do i=1,N
c         write(*,*) i, countn(i), keeppart(i)
c         do j=1,countn(i)
c            write(*,*) i, j, nl(j,i)
c         enddo
c      enddo
...
c      write(*,*)    
      ...
      do i=1,N	
         if(keeppart(i).eq.1) then
            ikeep=ikeep+1
c            countn(ikeep)=countn(i)
            jkeep=0
c            do j=1,countn(ikeep)
            do j=1,countn(i)
               if(keeppart(nl(j,i)).eq.1) then
                  jkeep=jkeep+1
                  nl(jkeep,ikeep)=renumber(nl(j,i))
c                  write(*,*) i,j,nl(j,i),ikeep,jkeep,nl(jkeep,ikeep)
               endif
            enddo
            countn(ikeep)=jkeep
         endif
